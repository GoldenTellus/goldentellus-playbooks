...
| `/theory-new` | 开始理论对话 | 已形成可讨论的核心观点 |
| `/theory-extract` | 从当前对话或案例提炼理论 | 观点、论据与边界经用户确认 |
| `/theory-update {id}` | 更新既有知识文章 | 优先处理查重后的既有条目 |
| `/theory-status` | 查看当前理论提炼进度 | 展示观点、证据、边界和目标目录 |
| `/link {case-id} {knowledge-id}` | 建立案例与理论关联 | 两侧 frontmatter 和链接均校验通过 |
...
2. 案例必须确认授权、脱敏和公开范围；理论必须确认观点、证据和边界可公开。
